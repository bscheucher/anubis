INSERT INTO gehaltsinfo (name, kz, passwort, created_by)
VALUES ('Gehaltszettel', 'N', null, current_user),
       ('Kontoauszug', 'J', null, current_user),
       ('täglich', 'T', null, current_user),
       ('per E-Mail', 'E', null, current_user),
       ('Portal', 'P', null, current_user),
       ('Portal täglich', 'L', null, current_user);


alter table gehalt_info
    add column lhr_gehaltsinfo integer references gehaltsinfo (id);
alter table gehalt_info_history
    add column lhr_gehaltsinfo integer;
...
BEGIN
    IF (TG_OP = 'DELETE') THEN
        INSERT INTO gehalt_info_history (gehalt_info_id, vertragsdaten_id, verwendungsgruppe,
                                         ...
                                         uest_pauschale, deckungspruefung, jobticket, lehrjahr, notiz_gehalt,
                                         lhr_gehaltsinfo, status, created_on,
        ...
        VALUES (OLD.id, OLD.vertragsdaten_id, OLD.verwendungsgruppe,
                ...
                OLD.uest_pauschale, OLD.deckungspruefung, OLD.jobticket, OLD.lehrjahr, OLD.notiz_gehalt,
                OLD.lhr_gehaltsinfo, OLD.status, OLD.created_on,
                OLD.created_by,
                OLD.changed_on,
    ...
    ELSIF (TG_OP = 'UPDATE') THEN
        ...
        VALUES (NEW.id, NEW.vertragsdaten_id, NEW.verwendungsgruppe,
                ...
                NEW.uest_pauschale, NEW.deckungspruefung, NEW.jobticket, NEW.lehrjahr, NEW.notiz_gehalt,
                NEW.lhr_gehaltsinfo, NEW.status, NEW.created_on,
                NEW.created_by,
                NEW.changed_on,

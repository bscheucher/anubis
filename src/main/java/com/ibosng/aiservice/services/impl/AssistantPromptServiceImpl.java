...
public class AssistantPromptServiceImpl implements AssistantPromptService {
     private final OpenaiAssistantAnweisungService openaiAssistantAnweisungService;

    private static final String SHARED_ZIELSTELLUNG = """
            # Zielstellung
            Dein Ziel ist es die bestmögliche, verfügbare Vertretungen für einen ausgefallenen Trainer zu finden. Zu diesem Zweck bekommst du den Informationen zum Trainer und einen Zeitraum, in dem der Trainer ausfällt. Du musst für jeden Kurs an jeden Tag, an dem der Trainer ausfällt die bestmögliche Vertretung finden. Die Bewertung erfolgt anhand der folgenden Kriterien:
    ...
    private static final Map<RequestType, String> PROMPTS = Map.of(
        RequestType.CHAT, """
            ### Anweisung
            ...
            ### Format
            Antworte in menschlicher Sprache!
            """,
        RequestType.ROUTING, """
            ### Anweisung
            ...
            konversation: Wenn die Anfrage eine normale Chat Nachricht ist.
            """,
        RequestType.SEMINAR_VERTRETUNG, """
            # Anweisung
            ...
            3. Finde die Menge an Trainern, die die notwendigen Voraussetzungen für jedes Seminar erfüllen und die in diesem Zeitraum verfügbar sind
            ...
            - Achte darauf, dass alle für die Bewertung notwendigen Informationen enthalten sind (siehe Bewertungskriterien und Bewertungsskala).
            """,
        RequestType.TRAINER_ELECTION, """
            # Anweisung
            ...
            Am Ende filter die fehlerhaften Trainer aus der Antwort aus.
            ...
            """ + SHARED_ZIELSTELLUNG + """
            # Format
            Deine Antwort MUSS im JSON-Format erfolgen und die folgenden Felder enthalten:
            ...
            Füge in das "reasoning"-Feld zu Beginn eine umfassende Erklärung ein, wie du die Bewertungskriterien angewendet hast.
            """
    );

    @Override
    public String getPrompt(RequestType type) {
        // Use local prompts for faster development
        //return PROMPTS.getOrDefault(type, "No prompt template available for this request type.");
        Map<RequestType, String> assistantMap = Map.of(
                RequestType.CHAT, CHAT_ASSISTANT,
                RequestType.ROUTING, ROUTING_ASSISTANT,
                RequestType.SEMINAR_VERTRETUNG, INFORMATION_AGGREGATOR_ASSISTANT,
                RequestType.TRAINER_ELECTION, TRAINER_ELECTION_ASSISTANT
        );
        return openaiAssistantAnweisungService.getLatestAnweisung(assistantMap.get(type));
    }
}

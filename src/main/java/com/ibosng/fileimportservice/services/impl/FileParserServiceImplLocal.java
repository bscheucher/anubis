...
/**
 * IMPORTANT: this is a 99% duplicate of {@link  FileParserServiceImpl} for the {@code localdev} and {@code test} Spring profiles.
 * The reason is to keep the implementations cleanly separated.
 */
@Profile({"localdev", "test"})
@Slf4j
@Service
@RequiredArgsConstructor
public class FileParserServiceImplLocal implements FileParserService {
    ...
    public void manageFiles(DriveItemCollectionPage items) {
        ...
        for (DriveItem item : items.getCurrentPage()) {
            ...
            // Capture the moved item so we can use the new id (works for both local and real SharePoint)
            DriveItem movedItem = sharePointService.moveFile(item.id, filename, sharePointProperties.getProcessing());
            String newId = movedItem.id;

            File file = null;
            try {
                // Use the new id returned by moveFile
                file = sharePointService.downloadFile(newId, filename);
            ...
            if (file != null) {
                files.add(new FileDetails(
                        ...
                        newId, // store the new id
                        FilenameUtils.getExtension(item.name).equals(EXCEL_EXTENSION)
                ));
            }
        }
        if (!files.isEmpty()) {

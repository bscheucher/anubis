...
public class FileParserServiceImplLocal implements FileParserService {
    ...
    public void manageFiles(DriveItemCollectionPage items) {
        ...
        for (DriveItem item : items.getCurrentPage()) {
            ...
            // Capture the moved item so we can use the new id (works for both local and real OneDrive)
            DriveItem movedItem = oneDriveService.moveFile(item.id, filename, oneDriveProperties.getProcessing());
            String newId = movedItem.id;

            File file = null;
            try {
                // Use the new id returned by moveFile
                file = oneDriveService.downloadFile(newId, filename);

    ...
    public boolean executeValidation(UnterhaltsberechtigteDto unterhaltsberechtigteDto, Unterhaltsberechtigte unterhaltsberechtigte) {
        if (!isNullOrBlank(unterhaltsberechtigteDto.getUNachname())) {
            ...
            if (result) {
                ...
                unterhaltsberechtigte.addError("unachname", "Das Feld ist ungültig", globalUserHolder.getUsername());
                return false;
            }
        }
        unterhaltsberechtigte.addError("unachname", "Das Feld ist leer", globalUserHolder.getUsername());
        return false;
    }
}

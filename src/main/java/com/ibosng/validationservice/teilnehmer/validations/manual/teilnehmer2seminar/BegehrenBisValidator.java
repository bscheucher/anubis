...
public class BegehrenBisValidator implements FieldValidator<SeminarDto> {
    @Override
    public void validate(SeminarDto dto, Integer parentId) {
        if (isNullOrBlank(dto.getBegehrenBis())) return;
        if (!isValidDate(dto.getBegehrenBis())) {
            dto.getErrorsMap().put("begehrenBis", "Ungültiges Datum");
        }
    }
}